...
function getWeekNumberByDate(date) {
  const fullYear = new Date(date.getFullYear(), 0);
  let week = 1;
  while (fullYear.getTime() !== date.getTime()) {
    const d = fullYear.getDate() + 1;
    fullYear.setDate(d);
    if (fullYear.getDay() === 0) week += 1;
  }
  return week;
}
